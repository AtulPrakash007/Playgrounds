...
struct RowsDataStore: Codable {
    var rows: [Rows]
}

func parseJsonData(data: Data) -> [Rows] {
    ...
    do {
        let rowDataStore = try decoder.decode(RowsDataStore.self, from: data)
        rows = rowDataStore.rows
    } catch {
        print(error)
    }
...
do {
    ...
    let rows = parseJsonData(data: data)
//    let json = try JSONSerialization.jsonObject(with: data, options: [])
//    print(json)
    for row in rows {
        if (row.title ?? "").isEmpty && (row.description ?? "").isEmpty && (row.imageHref ?? "").isEmpty { // use this in table view to skip the cell
            print(row)
        }
    }
} catch {
